...
function create() {
    ...
    groundGraphics.moveTo(0, 500); // Move ground down to y = 500
    groundGraphics.lineTo(800, 500); // Ending point (x, y)
    groundGraphics.strokePath();
    groundGraphics.fillStyle(0x8B4513, 1); // Brown color to fill the ground
    groundGraphics.fillRect(0, 490, 800, 20); // Draw a filled rectangle for the ground (adjusted Y)
    / Ground (visual representation and collision detection using a static physics object)
ground = this.physics.add.staticGroup();

// Create the ground (static, with collision enabled)
const groundObject = ground.create(400, 500, 'transparent'); // Create an invisible object (or a visible one if needed)
groundObject.setDisplaySize(800, 20); // Set the size of the ground
groundObject.setSize(800, 20); // Set the collision area size

// Optionally, add a visual rectangle (can be filled with brown color or left invisible)
const groundVisual = this.add.rectangle(400, 500, 800, 20, 0x8B4513);
groundVisual.setOrigin(0.5, 0.5); // Center the rectangle
