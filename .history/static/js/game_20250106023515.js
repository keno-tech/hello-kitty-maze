...
function create() {
    ...
    this.add.rectangle(400, 200, 1800, 4000, 0x87ceeb); // Sky-blue background
// Ground (Invisible, only for collision detection)
ground = this.physics.add.staticGroup();

// Create the collision object for the ground (just for physics, no visual element here)
ground.create(400, 500, null).setSize(800, 10); // Adjust the Y position and size as needed

// Optionally, if you want a visual representation for the ground without using graphics, 
// you could use a simple sprite or image. Here's an example of using a transparent image as the ground:
const groundImage = this.add.image(400, 500, 'transparent'); // Use a transparent image for visualization
groundImage.setOrigin(0.5, 0.5); // Center it properly
groundImage.setDisplaySize(800, 10); // Set the size (same as the collision object)

// Set the ground image to follow the physics world, without adding any physics body to it
groundImage.setVisible(false); // Optionally make it invisible if you don't need to see the ground
...
function spawnObstacle() {
    ...
    if (obstacleType === 0) {
        ...
        obstacle.body.setSize(1400, 1500);  // Adjust the width and height as needed
...
function showLeaderboardForm() {
    ...
    // Add event listener to the Replay button to refresh the page
